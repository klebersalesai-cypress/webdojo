describe('Validações de Alertas em JavaScript', ()=>{
    ...
    it('Deve confirmar um diálogo e validar a resposta positiva', ()=>{
        cy.on('window:confirm', (msg)=>{
            ...
            return true;//True simula o click no botao OK
        })
    ...
    it('Deve cancelar um diálogo e validar a resposta negativa', ()=>{
        cy.on('window:confirm', (msg)=>{
            expect(msg).to.equal('Aperte um botão!')
            return false;//True simula o click no botao Cancelar
        })
    ...
    it('Deve interagir com um prompy, inserir um texto e validar uma mensagem', ()=>{
        cy.window().then((win)=>{
            cy.stub(win, 'prompt').returns('kleber')
        })

        cy.on('window:alert', (msg)=> {
            expect(msg).to.equal('Olá kleber! Boas-vindas ao WebDojo!')
        })
